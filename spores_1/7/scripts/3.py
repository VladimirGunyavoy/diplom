...
from src.ui_setup import UI_setup
...
ui_setup = UI_setup(color_manager, ui_manager)

print("\n🎨 1. UI_setup и UIManager созданы")
print("   ✓ Готовы к настройке демонстрационного интерфейса")

# ===== СОЗДАНИЕ СЦЕНЫ И ДРУГИХ СИСТЕМ =====
scene_setup = SceneSetup(
    init_position=(1.5, -0.5, -0.5),
    init_rotation_x=37,
    init_rotation_y=-30,
...
start_spore = Spore(
    ...
    goal_position=goal_position,
    scale=0.03,
    position=[start_position[0], 0, start_position[1]],
    color_manager=color_manager
)
...
data_providers = {
    ...
    'get_param_info': lambda: (param_manager.param, param_manager.show)
}
...
spore_manager.add_spore(start_spore)
spore_manager.add_spore(goal)

zoom_manager.register_object(goal)
zoom_manager.register_object(start_spore)
...
print(f"   📍 Стартовая спора в позиции: {start_spore.position}")
