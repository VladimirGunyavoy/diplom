...
# create scene setup
scene_setup = SceneSetup(init_position=(0.263, -1.451, 0.116), init_rotation_x=42.750, init_rotation_y=-12.135)
frame = Frame()
scene_setup.frame = frame  # Добавляем frame в scene_setup
window_manager = WindowManager()

settings_param = ParamManager(0, show=False)

# create pendulum
pendulum = PendulumSystem(dt=0.2, damping=0.2)
...
spore_manager = SporeManager(pendulum=pendulum, zoom_manager=zoom_manager, settings_param=settings_param)
...
spore = Spore(pendulum=pendulum, dt=0.05, 
                              scale=ball_size,
                              position=(0, 0, 1))
