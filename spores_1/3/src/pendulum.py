...
class PendulumSystem:
    ...
    def __init__(self, dt=0.1, damping=0.1, max_control=1, verbose=False):
        # Параметры маятника
    ...
    def discrete_step(self, state, control=0):
        """
        Один шаг дискретной системы
        x[k+1] = A_d * x[k] + B_d * u[k]
        """
        # Преобразуем входные данные в numpy-массивы нужной формы (вектор-столбец)
        
        state_col = np.asarray(state).reshape(-1, 1)
        control_col = np.asarray(control).reshape(-1, 1)

        # Вычисляем следующее состояние
        next_state = self.A_discrete @ state_col + self.B_discrete @ control_col
        
        # Возвращаем результат в виде плоского массива (1D)
