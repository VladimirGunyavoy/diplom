...
class SporeLogic:
    ...
    def calculate_cost(self, position_2d: np.array = None) -> float:
        """Вычисляет стоимость для заданной позиции"""
        if position_2d is None:
            position_2d = self.position_2d
        return self.cost_function(position_2d)
    ...
    def evolve(self, state: np.array = None, control: float = 0) -> np.array:
        ...
        next_state = self.pendulum.discrete_step(state, control, self.dt)
        return next_state
