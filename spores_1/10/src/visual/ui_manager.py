...
class UIManager:
    ...
    def create_element(self, category: str, name: str, 
        ...
        should_have_background = style_info.pop('has_background', False)
        
        element_kwargs = style_info
    ...
    def toggle_element(self, name: str):
        element = self.get_element(name)
        if element:
    ...
    def create_position_info(self, name: str = 'main', position: tuple = UI_POSITIONS.POSITION_INFO):
        return self.create_element('dynamic', name,
            text="Position: 0.000, 0.000, 0.000\nRotation: 0.000, 0.000, 0.000",
            position=position, style='status')
    
    def create_instructions(self, name: str, instructions_text: str, position: tuple = UI_POSITIONS.DEFAULT_INSTRUCTIONS):
        return self.create_element('static', name,
            text=instructions_text, position=position, style='instructions')
    
    def create_status_indicator(self, name: str, initial_text: str = "", position: tuple = UI_POSITIONS.CURSOR_STATUS):
        return self.create_element('dynamic', name,
            text=initial_text, position=position, style='status', origin=(0, 0))
    
    def create_counter(self, name: str, initial_value: int = 0, position: tuple = UI_POSITIONS.DEFAULT_COUNTER, prefix: str = ""):
    ...
    def update_counter(self, name: str, value: int, prefix: str = ""):
        text = f"{prefix}{value}" if prefix else str(value)
        self.update_text(name, text)
    
    def create_info_block(self, name: str, title: str, content: str = "", position: tuple = UI_POSITIONS.DEFAULT_INFO_BLOCK, is_dynamic: bool = False):
    ...
    def update_info_block(self, name: str, title: str, content: str = ""):
        text = f"{title}\n{content}" if content else title
        self.update_text(name, text)
    
    def create_debug_info(self, name: str, position: tuple = UI_POSITIONS.DEFAULT_DEBUG_INFO):
