...
# Список спор (строка, столбец, может_достичь_финиша)
spores = []
...
def generate_spore_near_finish():
    ...
    while attempts < max_attempts:
        ...
        if is_valid_spore_position(row, col):
            grid[row][col] = 4
            spores.append([row, col, False])  # строка, столбец, может_достичь_финиша
            return True
...
def generate_spore_near_existing():
    ...
    if has_path_near_start:
        ...
        while attempts < 50:
            ...
            if is_valid_spore_position(row, col):
                grid[row][col] = 4
                spores.append([row, col, False])
                return True
            attempts += 1
    
    # Размещаем рядом с существующими достижимыми спорами, если возможно
    reachable_spores = [s for s in spores if s[2]]
    if reachable_spores:
        attempts = 0
        while attempts < 50:
    ...
    # В противном случае размещаем рядом с любой существующей спорой
    attempts = 0
    while attempts < 50:
        spore = random.choice(spores)
        offset_row = int(np.random.normal(0, 5))
        offset_col = int(np.random.normal(0, 5))
        row = spore[0] + offset_row
        col = spore[1] + offset_col
...
def run_agent():
    global current_spore_index, mode, agent_paths
        ...
        for step in range(10):
            ...
            if is_valid_position(next_row, next_col):
                ...
                for other_spore in spores:
                    if other_spore[2] and (next_row, next_col) == (other_spore[0], other_spore[1]):
    ...
    # Добавляем пути этой споры в общий список, только если спора успешна
    if reached_finish_or_reachable_spore:
        agent_paths.extend(current_spore_paths)
    # Если спора не может достичь финиша, удаляем её
    elif not spores[current_spore_index][2]:
        grid[spore[0]][spore[1]] = 0
        spores.pop(current_spore_index)
        current_spore_index -= 1  # Корректируем индекс, так как удалили спору
...
def update_reachable_spores():
    ...
    reachable_spores = [i for i, spore in enumerate(spores) if spore[2]]
    ...
    while newly_reachable:
        ...
        for i, spore in enumerate(spores):
            if i not in reachable_spores:  # Проверяем только недостижимые споры
                for path in agent_paths:
...
def check_for_path():
    ...
    if not has_path_near_start or not any(spore[2] for spore in spores):
        return False
    ...
    while queue:
        ...
        # Добавляем обычных соседей (вверх, вниз, влево, вправо)
        row, col = current
        ...
        for next_pos in neighbors:
            ...
            if (is_valid_position(r, c) and 
                ...
                (grid[r][c] == 0 or grid[r][c] == 3 or 
                 (grid[r][c] == 4 and any(s[0] == r and s[1] == c and s[2] for s in spores)))):
                queue.append(next_pos)
                visited[next_pos] = current
...
def reset_simulation():
    global grid, spores, agent_paths, successful_paths, can_reach_finish, final_path, current_spore_index, mode
...
def place_walls():
    # Размещаем больше случайных стен
    for _ in range(150):  # Увеличили количество стен
...
def main():
    global final_path, current_spore_index, mode
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Размещаем/удаляем стену
                mouse_pos = pygame.mouse.get_pos()
                col = mouse_pos[0] // GRID_SIZE
                row = mouse_pos[1] // GRID_SIZE
                
                if (row, col) != start_pos and (row, col) != finish_pos:
                    if grid[row][col] == 0:
                        grid[row][col] = 1
                    elif grid[row][col] == 1:
                        grid[row][col] = 0
