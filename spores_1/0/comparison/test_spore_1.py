...
def visualize_map(grid, start_pos, finish_pos, path=None, explored=None, spores=None, title=None):
    """Визуализирует карту с улучшенной легендой"""
    ...
    if explored:
        for x, y in explored:
            if grid_vis[x, y] == 0:  # Не перезаписываем старт, финиш и другие специальные клетки
                grid_vis[x, y] = 5  # 5 - исследованная клетка
    
    # Отмечаем путь (отметим до начала и финиша)
    if path and len(path) > 0:
        print(f"Отображаем путь длиной {len(path)} точек")
        for x, y in path:
            if grid_vis[x, y] != 2 and grid_vis[x, y] != 3:  # Не перезаписываем старт и финиш
                grid_vis[x, y] = 7  # 7 - путь
    
    # Отмечаем споры (после пути, чтобы они были видны поверх)
    if spores:
        for spore in spores:
            x, y = spore[0], spore[1]
            if grid_vis[x, y] != 2 and grid_vis[x, y] != 3:  # Не перезаписываем старт и финиш
    ...
    legend_elements = [
        ...
        mpatches.Patch(color='purple', label='Достижимая спора'),
        mpatches.Patch(color='orange', label='Путь')
    ]
...
class ImprovedSporeAgent:
    ...
    def reconstruct_path(self):
        ...
        if self.debug:
            print("  Используем построение пути через ключевые точки...")
...
def main():
    ...
    visualize = True  # Отключаем визуализацию результата
    visualize_steps = True  # Отключаем визуализацию промежуточных шагов
    max_iterations = 1000
