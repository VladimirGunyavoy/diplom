...
from src.scene.pillar import Pillar
import numpy as np
...
class AngelManager:
    ...
    def __init__(self, color_manager=None, zoom_manager=None, config=None):
        ...
        self.cost_surface = None # Будет установлено извне
        self.config = config
    ...
    def on_spore_created(self, spore):
        ...
        if (hasattr(spore, 'is_ghost') and spore.is_ghost) or not self.cost_surface:
            return

        # --- Код ниже выполняется только для НАСТОЯЩИХ спор ---

        pillar_width = self.config['angel']['pillar_width']
        spore_2d_pos = spore.calc_2d_pos()
        display_height = self.cost_surface.get_cost(spore_2d_pos)
    ...
    def create_ghost_visuals(self, ghost_spores):
        ...
        if not self.cost_surface or not ghost_spores:
            return
        ...
        pillar_width = self.config['angel'].get('ghost_pillar_width', self.config['angel']['pillar_width'])
    ...
    def update_ghost_angel_visuals(self, ghost_spores):
        ...
        if not self.cost_surface or len(ghost_spores) != len(self.ghost_angels):
            return
