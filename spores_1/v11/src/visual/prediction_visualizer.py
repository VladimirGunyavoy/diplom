...
class PredictionVisualizer:
    ...
    def update(self, predicted_state_2d: np.ndarray) -> None:
        ...
        self.ghost_spore.real_position = np.array([predicted_state_2d[0], 0.0, predicted_state_2d[1]])
        ...
        # 3. Обновляем ангела
        if self.angel:
            self.angel.real_position = self.ghost_spore.real_position + np.array([0, display_height + y_offset, 0])
        ...
        # 4. Обновляем столб
        if self.pillar:
            self.pillar.real_position = self.ghost_spore.real_position + np.array([0, (display_height + y_offset) / 2, 0])
            self.pillar.real_scale = np.array([self.pillar_width, display_height + y_offset, self.pillar_width])
