...
picker_manager = PickerManager(
    ...
    verbose_output=False  # Отключаем отладочный вывод при движении мыши
)
...
def input(key):
    ...
    # Обработка F11 для переключения полноэкранного режима
    if key == 'f11':
        window_manager.toggle_fullscreen()
        print(f"   🖥️ Полноэкранный режим: {'включен' if window_manager.is_fullscreen() else 'выключен'}")
        return
...
print("\n📋 ДОСТУПНЫЕ КОМАНДЫ:")
print("   СПРАВКА: / (показать/скрыть окно управления)")
print("   ОБЫЧНЫЕ: WASD, Space/Shift, мышь, Alt, Q")
print("   ПОЛНОЭКРАННЫЙ РЕЖИМ: F11 (переключение)")
print("   СПОРЫ: (устаревшие команды удалены)")
...
print("   ВРЕМЯ: [ (сброс dt), J (статистика dt)")
print("   ИЗОБРАЖЕНИЯ: M (генерация всех отладочных картинок)")
