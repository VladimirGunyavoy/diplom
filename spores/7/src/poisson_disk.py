...
class PoissonDiskSampler:
    ...
    """
    def __init__(self, min_radius, n_dim, k=30):
        """
        ...
        self.cell_size = self.min_r / np.sqrt(self.n_dim)
        
        # Размер сетки. Домен [-1, 1], поэтому его размер 2.
        self.grid_size = int(np.ceil(2.0 / self.cell_size))
        
        # Инициализируем сетку. -1 означает, что ячейка пуста.
        self.grid = -np.ones((self.grid_size,) * self.n_dim, dtype=int)
    ...
    def _generate_candidate(self, p):
