#!/usr/bin/env python3
"""
Тест форсированного пересчёта и гарантии подписки.
"""

import json
import numpy as np

def test_forced_recalculation():
    """Тестирует форсированный пересчёт и гарантию подписки."""
    print("🧪 ТЕСТ ФОРСИРОВАННОГО ПЕРЕСЧЁТА И ГАРАНТИИ ПОДПИСКИ")
    print("=" * 60)
    ...
    # Тестируем форсированный пересчёт
    print(f"\n🔧 ТЕСТИРУЕМ ФОРСИРОВАННЫЙ ПЕРЕСЧЁТ:")
    ...
    # Тестируем увеличение dt с форсированным пересчётом
    print(f"\n📝 Тестируем увеличение dt с форсированным пересчётом:")
    ...
    if mock_manual_spore_manager.ghost_tree_dt_vector is not None:
        ...
        if signs_preserved and scale_correct:
            print("✅ Форсированный пересчёт работает корректно!")
        else:
            print("❌ Ошибка в форсированном пересчёте")
    else:
    ...
    print(f"\n🎉 ТЕСТ ФОРСИРОВАННОГО ПЕРЕСЧЁТА ЗАВЕРШЕН!")
    print("📋 Проверьте логи выше на наличие:")
    print("   [IM] Ctrl+Wheel: forcing _on_dt_changed()")
    print("   [IM] re-subscribed to DTManager.on_change after pairing")
    print("   [IM] ghost_dt_baseline set to ... after pairing")
    print("   [DT] subscribers for id=...")

if __name__ == "__main__":
    test_forced_recalculation()
