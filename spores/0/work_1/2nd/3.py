...
class Simulation:
    def __init__(self):
        ...
        self.removed_spores = 0  # Счетчик удаленных спор
    ...
    def update_all_spores_status(self):
        ...
        # Удаляем споры, которые не ведут к финишу
        spores_to_remove = []
        for spore in self.spores:
            if not spore.reachable:
                spores_to_remove.append(spore)
                x, y = spore.x, spore.y
                # Заменяем цвет споры на голубой (как обычная посещенная клетка)
                self.grid[y][x].color = BLUE
                self.grid[y][x].is_spore = False
        
        # Удаляем не ведущие к финишу споры из списка
        for spore in spores_to_remove:
            if spore in self.spores:  # Проверка, чтобы избежать ошибок
                self.spores.remove(spore)
                self.removed_spores += 1
...
def main():
    ...
    while running:
        ...
        # Отрисовка сетки (с учетом смещения из-за информационной панели)
        ...
        if sim.final_path:
            for i in range(len(sim.final_path) - 1):
                ...
                # Рисуем линию между точками пути (с учетом смещения)
                info_panel_height = 80
                start_screen_pos = (start_pos[0] * CELL_SIZE + CELL_SIZE // 2, 
                                   start_pos[1] * CELL_SIZE + CELL_SIZE // 2 + info_panel_height)
                end_screen_pos = (end_pos[0] * CELL_SIZE + CELL_SIZE // 2, 
                                 end_pos[1] * CELL_SIZE + CELL_SIZE // 2 + info_panel_height)
                pygame.draw.line(screen, GREEN, start_screen_pos, end_screen_pos, 3)
        
        # Создаем информационную панель
        info_panel_height = 80
        info_panel = pygame.Surface((WIDTH, info_panel_height))
        info_panel.fill((240, 240, 240))  # Светло-серый фон для панели
        pygame.draw.line(info_panel, BLACK, (0, info_panel_height-1), (WIDTH, info_panel_height-1), 2)

        # Отображение информации
        info_text = [
            f"Спор: {len(sim.spores)}, Достижимых: {len(sim.reachable_spores)}",
            f"Найден путь: {'Да' if sim.final_path else 'Нет'}",
            f"Удалено неэффективных спор: {sim.removed_spores}",
            "N - создать спору, R - сбросить симуляцию"
        ]
        
        for i, text in enumerate(info_text):
            text_surface = font.render(text, True, BLACK)
            info_panel.blit(text_surface, (10, 10 + i * 20))
            
        # Размещаем информационную панель в верхней части экрана
