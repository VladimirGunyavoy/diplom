...
def generate_spore_near_existing():
    ...
    if random.random() < 0.2:
        # Получаем соседние клетки от старта (только прямые соседи: вверх, вниз, влево, вправо)
        start_row, start_col = start_pos
        neighbors = [(start_row+dr, start_col+dc) for dr, dc in [(0,1), (1,0), (0,-1), (-1,0)]]
        random.shuffle(neighbors)  # Перемешиваем соседей для случайности
        
        # Пытаемся разместить спору на одной из соседних клеток
        for neighbor_row, neighbor_col in neighbors:
            if is_valid_spore_position(neighbor_row, neighbor_col):
                grid[neighbor_row][neighbor_col] = 4
                spores.append([neighbor_row, neighbor_col, False, False])
                return True
...
def place_walls():
    # Размещаем стены
    for _ in range(80):  # Уменьшено количество стен до 100
